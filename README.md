...
The Collatz corona, in the terminology of Belaga--Mignotte, is the finite set of cycle coefficients `C(y, sigma)` generated by admissible exponent patterns in the accelerated Collatz cycle equation.
...
The central local-divisibility example is `(x,y) = (24,15)`, where:
...
where `A_q` is the set of admissible patterns such that `q` divides `C(15, sigma)`.

A naive local-independence benchmark would predict approximately:

```text
|A_13| · |A_186793| / |Sigma(24,15)| ≈ 3.8409
```
...
Run the independent Python verification of the critical case `(x,y) = (27,17)`:
...
The local-divisibility data for the central case `(x,y) = (24,15)` are in:
...
For every `(x,y)` with `2^x > 3^y` and `(x,y) != (2y,y)`, the conjecture tested here is:

```text
{ C(y, sigma) : sigma in Sigma(x,y) } ∩ (2^x - 3^y)Z = empty
```
